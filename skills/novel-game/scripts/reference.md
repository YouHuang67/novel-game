...
# Agent prompt 已在 split_batches.py 生成（batch_NNN_prompt.md）
# 每个agent读取prompt，输出batch_NNN.json到 _batches/ 目录
# 并行启动全部batch agent（14个batch=14个并行agent）
```

每个 batch agent 提取的 JSON 包含：
- scenes: 每个场景的章节、行号、地点、人物、事件、等级变化
- key_events: 本批最重要的3-5个剧情事件
- new_characters: 本批首次出场的人物
- new_locations: 本批首次出现的地点
- power_progression: 本批内的实力变化总结
...
自动生成：
- `lore/plot/timeline.md` — 443+场景完整时间线，行号定位
- `lore/plot/arcs.md` — 按地图分段的大剧情弧线
- `lore/plot/map_transitions.md` — 地图切换事件表
- `lore/characters/INDEX.md` — 300+人物汇总，含首次出场batch
